...
class Lammps:
    ...
    def Write_data(self, filepath, Item_list):
    ...
    def Write_Lammps(self, filepath):
...
def Convert_Amber_files(crd_filepath, top_filepath, data_filepath):
    ...
    a = Amber()
    a.Read_CRD(crd_filepath)
    if a.CRD_is_read:
        a.Read_TOP(top_filepath)
        if a.TOP_is_read:
            l = a.Coerce_to_Lammps()
            l.Write_Lammps(data_filepath)
            del l
    del a
    print
