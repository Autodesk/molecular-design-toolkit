...
class MolecularAtomList(AtomListOperationMixin, utils.AutoIndexList):
    ...
    def __init__(self, atoms, mol):
        super(MolecularAtomList, self).__init__(atoms)
        self._mol = mol

    def _error_if_atom_owned(self, atom):
        if atom.molecule is self._mol:
            assert atom in self
            raise ValueError("Atom %s cannot appear twice in this molecule" % atom)
        elif atom.molecule is not None:
            raise ValueError("Atom %s is already a member of %s" % (atom, atom.molecule))
        elif atom.residue is not None:
            raise ValueError("Cannot individually assign %s to %s, because it is part of %s" %
                             (atom, self._mol, atom.residue))

    def _own_atom(self, atom):
        atom._molecule = self._mol
        atom._position = None
        atom._momentum = None

    def _disown_atom(self, atom):
        assert self._mol is atom._molecule

        atom._molecule = None

    def append(self, atom):
        """ Adds atom to this molecule. Equivalent to ``atom.molecule = mol``

        If the atom is already assigned to a residue, the residue will be assigned to this molecule
        as well.

        Args:
            atom (moldesign.Atom): atom to add

        Raises:
            ValueError: If atom is already part of a different molecule
        """
        self._error_if_atom_owned(atom)
        super(MolecularAtomList, self).append(atom)
        self._own_atom(atom)

    def insert(self, index, atom):
        self._error_if_atom_owned(atom)
        self._own_atom(atom)
        super(MolecularAtomList, self).insert(index, atom)

    def remove(self, atom):
        """ Removes atom from this molecule. Equivalent to running ``atom.molecule = None``

        The atom is automatically removed from its residue as well.

        Args:
            atom (moldesign.Atom): atom to remove

        Raises:
            ValueError: If atom is not part of the molecule
        """
        assert atom is self[atom.index]
        self.pop(atom.index)

    def pop(self, index=-1):
        self._disown_atom(self[index])
        return super(MolecularAtomList, self).pop(index)
